...
def calcul(event=None):   
    ...
    try:
        ...
        correction.config(text="please enter correct number ! (1; 2; 3; 4; 5; 6; 7; 8; 9)", bg=color, fg='red', font=("impact", 9))
        correction.pack()
    else:
        if correction:
...
def choose_unit_screen():
    ...
    def update_radio_buttons():
        ...
        if current_unit == "kg":
            ...
            radio_lbs.config(bg=color)
        elif current_unit == "lbs":
            radio_lbs.config(bg='grey')
            radio_kg.config(bg=color)
    def kg_selected():
        weight_unit.set("kg")
...
def kg_to_lbs():
    try:
        ...
        entry_lbs.insert(0, str(round(lbs, 1)))
    except ValueError:
        pass
...
def lbs_to_kg():
    try:
        ...
        entry_kg.insert(0, str(round(kg, 1)))
    except ValueError:
        pass
...
frame = tk.Frame(window, bg=color, bd=16, width=400, height=300)
frame.grid(row=0, column=0, sticky="nsew")
#frame2
frame2 = tk.Frame(window, bg=color, bd=16, width=400, height=1080)
...
#conversion d'unité affichage
label_kg = tk.Label(frame, text="Kilogrammes:", font=("impact", 7))
label_kg.grid(row=11, column=0)

entry_kg = tk.Entry(frame)
entry_kg.config(font=("impact", 7))
entry_kg.grid(row=11, column=1)
entry_kg.bind("<KeyRelease>", lambda event: kg_to_lbs())

label_lbs = tk.Label(frame, text="Livres:", font=("impact", 7))
label_lbs.grid(row=12, column=0)

entry_lbs = tk.Entry(frame)
entry_lbs.config(font=("impact", 7))
...
label_valeur1.config(font=("impact", 12), bg=color, fg=color2)
label_valeur1.grid(row=0, column=0, pady=10)
entry_valeur1 = tk.Entry(frame)
entry_valeur1.grid(row=1, column=0, pady=10)
entry_valeur1.bind("<KeyRelease>", calcul)

label_valeur2 = tk.Label(frame, text="number of reps :")
label_valeur2.config(font=("impact", 12), bg=color, fg=color2)
label_valeur2.grid(row=2, column=0, pady=10)
entry_valeur2 = tk.Entry(frame)
entry_valeur2.grid(row=3, column=0, pady=10)
entry_valeur2.bind("<KeyRelease>", calcul)


#bouton help
bouton_help = tk.Button(frame2, text= "help", command=open_help_screen)
bouton_help.config(font=("impact", 10), borderwidth=5, highlightthickness=5)
bouton_help.grid(row=0, column=0)


#bouton unités
bouton_unit = tk.Button(frame2, text="choose unit", command=choose_unit_screen)
bouton_unit.config(font=("impact", 10), borderwidth=5, highlightthickness=5)
...
label_resultat.config(background=color, font=("impact", 9), fg=color2)
label_resultat.grid(row=5, column=0)

label_resultat_90 = tk.Label(frame, text="90% of 1RM : ")
label_resultat_90.config(background=color, font=("impact", 7), fg=color2)
label_resultat_90.grid(row=6, column=0)

label_resultat_80 = tk.Label(frame, text="80% of 1RM : ")
label_resultat_80.config(background=color, font=("impact", 7), fg=color2)
label_resultat_80.grid(row=7, column=0)

label_resultat_70 = tk.Label(frame, text="70% of 1RM : ")
label_resultat_70.config(background=color, font=("impact", 7), fg=color2)
label_resultat_70.grid(row=8, column=0)

label_resultat_60 = tk.Label(frame, text="60% of 1RM : ")
label_resultat_60.config(background=color, font=("impact", 7), fg=color2)
label_resultat_60.grid(row=9, column=0)

label_resultat_50 = tk.Label(frame, text="50% of 1RM : ")
label_resultat_50.config(background=color, font=("impact", 7), fg=color2)
label_resultat_50.grid(row=10, column=0)
